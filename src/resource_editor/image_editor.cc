#include "resource_editor/image_editor.hh"
...
namespace ResourceEditor
{
  ...
  SpriteView::draw()
  {
    ...
    {
      img_display_size = Vec2(container_size.y * img_ar, container_size.y);
    ...
    cursor_pos(0, 0), sprite(nullptr)
  {
    ...
    color_input = new ColorSelector();
    color_container = new FlexContainer("Color", color_input);
    add_child(color_container);

    palette_view = new SpriteView(Vec2(500, 32), Vec2(512, 512), b_pal);
    palette_container = new FlexContainer("Palette", palette_view);
    palette_container->set_origin(Vec2(600, 200));
    add_child(palette_container);

    img_view = new SpriteView(Vec2(500, 32), Vec2(512, 512), b_pal);
    img_container = new FlexContainer("Sprite", img_view);
    img_container->set_origin(Vec2(800, 200));
    add_child(img_container);
  }
